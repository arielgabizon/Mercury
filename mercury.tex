...
\subsection{Our results}
    ...
    We propose a new protocol called \mercury that goes beyond this tradeoff: a protocol with constant proof size and only $O(n)$ prover operations (besides the $O(\lambda n/(\log(\lambda n))$ operations for muti-scalar multiplications arising in KZG commitments).
    Additionally, \mecury is concretely more efficient than existing schemes in terms of required MSMs as can be seen in table \ref{table:comparison} 
