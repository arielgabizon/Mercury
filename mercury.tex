...
\newcommand{\B}[1]{\ensuremath{\mathcal{B}_{#1}}\xspace}
...
% \begin{adjustbox}{width=1\textwidth}
	...
	\thead{Scheme} & \thead{Proof size} & \thead{Prover Work} & \thead{Verifier Work}  \\ \hline
		...
		\mercury\cite{mercury}       & $O(1)$ \F   & $O(n)$ \F, $2n+ O(\sqrt n)$ \G     & $O(\log n)$ \F, $O(1)$ \G          \\ \hline
	\end{tabular}
% \end{adjustbox}
...
outputs $\cm=\sumi{n}f_i \enc{x^i}$ as a commitment to the multilinear function $M(X_0,\ldots,X_{s-1})= \sumi{n}\eq(i,u) f_i$ where $u\in \F^{\log n}$.
...
Thus, we can perform consistency checks between each pair $\cm_{i-1},\cm_i$ showing $\cm_i$ is indeed the commitment to the next correct restriction.
Of course, we get $O(s)=O(\log n)$ proof length due to sending this sequence of restriction commitments.
...
Note that when $g_1,g_2 \in \polysofdeg{b}$ the coefficients of $S(X)$
