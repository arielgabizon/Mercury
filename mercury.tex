...
\section{Introduction}
    ...
    A different class of SNARKs \cite{spartan, hyperplonk, honk} arising from the sumcheck protocol \cite{lfkn} have linear prover time, but require multilinear Polynomial Commitment Schemes (\mlpcs's).
    ...
    We propose a new protocol that reconciles this tradeoff: a protocol with constant proof size and linear prover time.
    Additionally, this is concretely more efficient than existing schemes, with a smaller constant factor of prover work ($1 + o(1)$ or $2 + o(1)$ compared to $2.5$ for Zeropmorph)  and concretely small proof sizes.
