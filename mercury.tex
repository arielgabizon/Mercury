...
\end{enumerate}
The new technical component is a ``folding'' of the witness via univariate polynomial division. As opposed to more familiar folding methods - e.g. as used in the sumcheck protocol; this method allows to fold the witness by a \emph{super-constant} factor with a \emph{linear} number of field operations and scalar multiplications.
\end{abstract}
...
\gemini commits to the multilinear function as a \emph{univariate}. Specifically,
suppose given a vector $a\in \F^n$, and a structured reference string of elements \sett{\enc{x^i}}{i<n}. \gemini
outputs $\sumi{n}a_i \enc{x^i}$ as a commitment to the multilinear function $\mle{a}(u)= \sumi{n}\eq(i,u) a_i$ where $u\in \F^{\log n}$.
...
\subsection{Polynomial commitment schemes for multi-linear polynomials}
