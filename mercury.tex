...
\begin{remark}
In hindsight, partially due to concurrent work\cite{samaritan}, we realized our protocol can be viewed as a sumcheck protocol\cite{lfkn} optimized for the \mlpcs setting. Some details on this viewpoint are given in Appendix \ref{subsec:sumcheck}.
\end{remark}
...
\begin{enumerate}
\item Let $u=(u_1,u_2)$ for $u_1,u_2 \in \F^{t}$. \prv computes the polynomial $h(X)\defeq \sumi{b}\eq(i,u_1) f_i(X)$. Note that the coefficient of $X^j$ in $h(X)$ is $\sumi{b}\eq(i,u_1) f_{i,j}$ - hence we think of it as a commitment to partial sums. 
...
We thank Benedikt B{\"{u}}nz and Tohru Kohrita for comments and corrections.
\bibliographystyle{alpha}
\bibliography{references}
\appendix
\section{A sumcheck perspective on \mercury}\label{subsec:sumcheck}
We sketch how one could arrive at a similar protocol by modifying the sumcheck protocol \cite{lfkn}.
...
To maintain constant proof size and linear prover time, we need to resolve three issues.
\begin{enumerate}
\item The first univariate $s_1(X)$ has the form $s_1(X)=\sumi{b} \eqpol{u_1}(X) f(X,i)$ where $\eqpol{u_1}(X)$ represents the function $\eq(u_1,X)$ (see Section \ref{sec:eq-polys}). The standard way to compute $s_1$ is by first computing $\eqpol{u_1}(X) f(X,i)$ for each $0\leq i <b$, and summing the results. This requires a $b$-size $FFT$ for each of the $b$ coordinates giving superlinear time $O(n\log b)$ in total. However, as \eqpol{u_1} depends only on $X$ we can simply compute the polynomial $\sumi{b} f(X,i)$, not requiring these FFTs, and multiply it in the end by \eqpol{u_1}, computing $s_1(X)$ in $O(n)$ time in total. In other words, when we are doing sumcheck over a product with a tensor vector like  \eq, computing $s_1$ can be done in linear time even when the domain is super-constant.
 ...
 coefficients.\footnote{We note that a bivariate sumcheck over large variable domains was handled similarly in \cite{BCGRS}.}
 \item The sumcheck verifier \ver needs to check the sum of $s_1$'s values on the first variable's domain is $v$. \ver can't do this check  directly from the commitment. This can be resolved by a univariate sumcheck subprotocol. A technical note is that \mercury uses the polynomial $h$ which actually matches $s_1/\eqpol{u_1}$. That is why an inner product subprotocol is needed (to take the inner product with \eqpol{u_1}) rather than merely a univariate sumcheck.

\end{enumerate}



 Finally, $f(X,Y)$ as a bivariate needs to be opened at the sumcheck challenge $(r_1,r_2)$. Thus, we need a bivariate PCS.  It turns out we have implicitly used in \mercury (a variant of) the bivariate PCS from \cite{samaritan} based on \cite{caulk}.
Specifically, $g(\zeta)$ in our main protocol in Section \ref{sec:main} is the evaluation 
$F(\zeta,\alpha)$; where $F(X,Y)$ is the bivariate polynomial such that our $f(X)$ is $F(X,X^b)$.
