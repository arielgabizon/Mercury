...
we assume an \emph{object generator} \obgen that is run with input $\lambda$ before all protocols, and returns all fields and groups used. Specifically, in our protocol $\obgen(\lambda) = (\F, \G, \Gtwo, \Gt, e, g_1, g_2,g_t)$ where
\begin{itemize}
...
We write \G and \Gtwo additively. We use the notations $\enc{x}\defeq x\cdot g_1$ and $\enctwo{x}\defeq x\cdot g_2$.
