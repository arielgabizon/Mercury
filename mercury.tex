...
\newcommand{\inprod}[2]{\ensuremath{\left\langle #1,#2 \right \rangle}\xspace}
\begin{document}
    \maketitle
\begin{abstract}
...
\begin{itemize}
 \item For $i\in \set{1,2}$, whenever \adv outputs an element $A\in \Gi$, it also outputs a vector $v$ over \F such that $A = \inprod{v}{\srsi}$.
\end{itemize}
...
We define $\inprod{g_1}{g_2}$ to be $\sum_{i=0}^d a_i b_i$ where $d\defeq \min \set{d_1,d_2}$. 
We present a convenient way to verify inner products $\inprod{g_1}{g_2}$ similar to \cite{bootle,sonic}.
The basic observation is that  $\inprod{g_1}{g_2}$ is the constant coefficient of the rational function
$R(X)\defeq g_1(X) g_2(1/X)$.
Thus, $\inprod{g_1}{g_2}=v$ is equivalent to the existence of polynomials $S_1(X),S_2(X)$ such that
...
$\inprod{g_1}{g_2}=v$ is equivalent to the existence of $S(X)\in \polys$ such that
...
Suppose we now have two  inner product claims  $\inprod{g_1}{g_2} =v_1$ and $\inprod{h_1}{h_2}=v_2$. 
...
\begin{claim}\label{claim:batchipa}
 ...
 Suppose $\inprod{g_1}{g_2}\neq v_1$ or $\inprod{h_1}{h_2}\neq v_2$. Then, e.w.p $1/|\F|$ over $\gamma\in \F$
...
\end{claim}
\begin{proof}
Denote $(v'_1,v'_2)=(\inprod{g_1}{g_2},\inprod{h_1}{h_2})$.
...
\[\inprod{\eqpol{u}}{g}=\sumi{b}\eq(i,u) g_i = \mle{g}(u).\]
...
\item \ver chooses a random $\zeta\in \Fstar$.
...
\item \ver chooses random $\zeta \in \Fstar$.
...
\begin{enumerate}
\item  \ver sends a random evaluation challenge $\zeta \in \Fstar$.
...
doesn't hold as a rational identity. Multiplying denominators, we have from the Schwarz-Zippel Lemma that $E_1$ occurs with probability at most $6n/|\F| = \negl$ over $\zeta \in \F$.
